...
Plan to implement                                                                                                                                                                                                                                                         │
│                                                                                                                                                                                                                                                                           │
│ Plan: Auto-derive PineAPPL channels from APFEL++ physics                                                                                                                                                                                                                  │
│                                                                                                                                                                                                                                                                           │
│ Context                                                                                                                                                                                                                                                                   │
│                                                                                                                                                                                                                                                                           │
│ Currently, grid_dis.yaml requires the user to manually specify Channels (e.g., [[2],[-2]] for u+ubar). This is error-prone: the grid_dis.yaml only had u, d, and gluon channels — missing s, c, b entirely. This caused the PineAPPL grid convolution to give only a      │
│ partial F2, not the full structure function.                                                                                                                                                                                                                              │
│                                                                                                                                                                                                                                                                           │
│ The channel structure is fully determined by the process, observable, and number of active flavors — APFEL++ already knows this. We should derive channels automatically and remove manual specification.                                                                 │
│                                                                                                                                                                                                                                                                           │
│ Physics rules (from APFEL++ DISNCBasis source):                                                                                                                                                                                                                           │
│ - F2/FL (C-even): For each active quark q: channel (q + qbar) with pids [[q],[-q]], factors [1,1]. Plus a gluon channel [[21]], factors [1].                                                                                                                              │
│ - F3 (C-odd): For each active quark q: channel (q - qbar) with pids [[q],[-q]], factors [1,-1]. No gluon channel (CG=0 at all orders).                                                                                                                                    │
│ - SIA: Same channel structure as DIS (reuses DISNCBasis).                                                                                                                                                                                                                 │
│ - nf_max: Determined by apfel::NF(sqrt(q2_max), thresholds) from the grid's maximum Q².                                                                                                                                                                                   │
│                                                                                                                                                                                                                                                                           │
│ Files to modify                                                                                                                                                                                                                                                           │
│                                                                                                                                                                                                                                                                           │
│ 1. pineapfel/include/grid_gen.h — add derive_channels declaration                                                                                                                                                                                                         │
│                                                                                                                                                                                                                                                                           │
│ std::vector<ChannelDef> derive_channels(Observable observable, int nf_max);                                                                                                                                                                                               │
│                                                                                                                                                                                                                                                                           │
│ 2. pineapfel/src/grid_gen.cpp — implement derive_channels + make Channels optional                                                                                                                                                                                        │
│                                                                                                                                                                                                                                                                           │
│ derive_channels(observable, nf_max):                                                                                                                                                                                                                                      │
│ - For F2/FL: nf_max quark channels {[[q],[-q]], [1,1]} for q=1..nf_max, plus gluon {[[21]], [1]}                                                                                                                                                                          │
│ - For F3: nf_max quark channels {[[q],[-q]], [1,-1]} for q=1..nf_max, no gluon                                                                                                                                                                                            │
│                                                                                                                                                                                                                                                                           │
│ load_grid_def(): Make Channels field optional. If absent, leave grid_def.channels empty.                                                                                                                                                                                  │
│                                                                                                                                                                                                                                                                           │
│ 3. pineapfel/src/fill.cpp — auto-derive channels in build_grid()                                                                                                                                                                                                          │
│                                                                                                                                                                                                                                                                           │
│ At the start of build_grid(), before creating the PineAPPL grid:                                                                                                                                                                                                          │
│ 1. Compute nf_max from max Q² across all bins and theory.quark_thresholds                                                                                                                                                                                                 │
│ 2. Always set grid_def.channels = derive_channels(grid_def.observable, nf_max), overriding any user-supplied channels                                                                                                                                                     │
│ 3. Log the derived channels                                                                                                                                                                                                                                               │
│                                                                                                                                                                                                                                                                           │
│ 4. runcards/grid_dis.yaml — remove Channels section                                                                                                                                                                                                                       │
│                                                                                                                                                                                                                                                                           │
│ Remove the entire Channels: block (lines 13-19). Channels will be auto-derived.                                                                                                                                                                                           │
│                                                                                                                                                                                                                                                                           │
│ 5. tests/test_grid_vs_apfelxx.cpp — add BuildStructureFunctions comparison                                                                                                                                                                                                │
│                                                                                                                                                                                                                                                                           │
│ The grid will now include all 5 active flavors + gluon. Add a new test that compares:                                                                                                                                                                                     │
│ - PineAPPL grid convolution (all orders, all auto-derived channels)                                                                                                                                                                                                       │
│ - APFEL++ BuildStructureFunctions (total F2 with all flavors, summed over same Q² nodes)                                                                                                                                                                                  │
│                                                                                                                                                                                                                                                                           │
│ These should now agree since the channel set is complete.                                                                                                                                                                                                                 │
│                                                                                                                                                                                                                                                                           │
│ 6. pineapfel_cli/build.cpp — no changes needed                                                                                                                                                                                                                            │
│                                                                                                                                                                                                                                                                           │
│ It calls build_grid() which will auto-derive channels internally.                                                                                                                                                                                                         │
│                                                                                                                                                                                                                                                                           │
│ Verification                                                                                                                                                                                                                                                              │
│                                                                                                                                                                                                                                                                           │
│ 1. cmake .. && make -j$(nproc) — builds without errors                                                                                                                                                                                                                    │
│ 2. ./build/pineapfel-build runcards/grid_dis.yaml runcards/theory.yaml runcards/operator.yaml — builds grid without Channels in YAML                                                                                                                                      │
│ 3. ./build/test_grid_vs_apfelxx — all tests pass, including new BuildStructureFunctions comparison                                                                                                                                                                        │
│ 4. ctest --output-on-failure — CTest passes

  1. pineapfel/include/grid_gen.h — Added derive_channels(Observable, int nf_max) declaration
  2. pineapfel/src/grid_gen.cpp — Two changes:
    - Implemented derive_channels(): generates quark channels {[[q],[-q]], [1,1]} for F2/FL or {[[q],[-q]], [1,-1]} for F3, plus gluon {[[21]], [1]} for F2/FL only
    - Made Channels YAML field optional in load_grid_def()
  3. pineapfel/src/fill.cpp — In build_grid(): computes nf_max from max Q² across bins, then always overrides channels with derive_channels(). Changed parameter name to grid_def_in and makes a mutable copy.
  4. runcards/grid_dis.yaml — Removed the entire Channels: block (was only u, d, gluon — missing s, c, b)
  5. tests/test_grid_vs_apfelxx.cpp — Two changes:
    - Added explicit derive_channels() call so the test reference computation uses the correct channels
    - Added TEST 4: compares PineAPPL grid convolution against APFEL++ BuildStructureFunctions (total F2 with all flavors), with 1% tolerance for interpolation differences
