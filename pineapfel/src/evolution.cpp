...
extern "C" void evolution_operators_qcd(std::size_t op_index,
    double                                          fac1,
    const int                                      *pids_in,
    const double                                   *x_in,
    const int                                      *pids_out,
    ...
    std::cout << "Convolution type: " << conv_type << ", mu = " << sqrt(fac1)
              << " GeV" << std::endl;
    ...
    // Zero operator
    const apfel::Operator    ZeroOp{op_params->DglapObj.at(conv_type)
                                     .begin()
                                     ->second.SplittingFunctions.begin()
                                     ->second.GetObjects()
                                     .begin()
                                     ->second.GetGrid(),
        apfel::Null{},
    ...
    const std::map<int, apfel::Operator>           EvOpPlusMinus =
        BuildDglap(op_params->DglapObj.at(conv_type),
            op_params->mu0,
            op_params->pert_ord,
            op_params->alphas)
            ->Evaluate(sqrt(fac1))
        ...
        for (int j = 0; j < 13; j++) {
            ...
            for (int k = 0; k < 13; k++) {
                for (int l = 0; l < 13; l++) {
                    ...
                    EvOpPhys.at({i, j}) +=
                        (apfel::RotPlusMinusToPhys[i][k] *
                            apfel::RotPhysToPlusMinus[l][j]) *
    ...
    for (std::size_t i = 0; i != flat_len; i++) {
        ...
        eko_buffer[i] = EvOpPhys.at(ij)
                            .Evaluate(x_in[coords[1]])
                            .GetDistributionJointGrid()[coords[3]] *
...
struct ApfelxxParamsQED {
    ...
    std::map<pineappl_conv_type, std::map<int, apfel::DglapObjectsQCDQED>>
        DglapObj;
};

extern "C" void evolution_operators_qed(std::size_t op_index,
    double                                          fac1,
    const int                                      *pids_in,
    const double                                   *x_in,
    const int                                      *pids_out,
        ...
        for (int j = 0; j < 20; j++) {
            ...
            for (int k = 0; k < 20; k++) {
                for (int l = 0; l < 20; l++) {
                    ...
                    EvOpPhys.at({i, j}) +=
                        (apfel::RotPlusMinusQCDQEDToPhys[i][k] *
                            apfel::RotPhysToPlusMinusQCDQED[l][j]) *
    ...
    for (std::size_t i = 0; i != flat_len; i++) {
        ...
        // Skip leptons
        if (std::abs(pids_in[coords[0]]) == 11 ||
            std::abs(pids_in[coords[0]]) == 13 ||
            std::abs(pids_in[coords[0]]) == 15 ||
            std::abs(pids_out[coords[2]]) == 11 ||
            std::abs(pids_out[coords[2]]) == 13 ||
            std::abs(pids_out[coords[2]]) == 15)
            continue;

        int ii = -1;
        if (pids_in[coords[0]] == 21) ii = 6;
        else if (pids_in[coords[0]] == 22) ii = 16;
        else ii = pids_in[coords[0]] + 6;

        int jj = -1;
        if (pids_out[coords[2]] == 21) jj = 6;
        else if (pids_out[coords[2]] == 22) jj = 16;
        else jj = pids_out[coords[2]] + 6;

        eko_buffer[i] = EvOpPhys.at({ii, jj})
                            .Evaluate(x_in[coords[1]])
                            .GetDistributionJointGrid()[coords[3]] *
                        x_out[coords[3]] / x_in[coords[1]];
    }
...
pineappl_grid *evolve(pineappl_grid *grid,
    ...
    for (std::size_t i = 0; i != n_convs; i++) {
        ...
        if (std::find(unique_convs.begin(), unique_convs.end(), conv) ==
            unique_convs.end()) {
    ...
    pineappl_grid_evolve_info(grid,
        nullptr,
        fac1.data(),
        frg1.data(),
        pids_in.data(),
    ...
    // EKO shape
    const std::vector<std::size_t>      shape    = {pids_in.size(),
                x_in.size(),
                pids_out.size(),
                x_out.size()};
    ...
    if (theory.qed) {
        ...
        apfel::AlphaQCDQED a{theory.alpha_qcd_ref,
            theory.alpha_qed_ref,
            theory.q_ref,
            theory.quark_thresholds,
            theory.lepton_thresholds,
            theory.pert_ord};
        const apfel::TabulateObject<apfel::matrix<double>> Couplings{a,
            tab.n_points,
            tab.q_min,
            static_cast<double>(tab.n_steps),
            tab.interp_degree};
        op_params->alphas = [=](double const &mu) -> double {
            return Couplings.Evaluate(mu)(0, 0);
        };
        op_params->alphaem = [=](double const &mu) -> double {
        ...
        fktable = pineappl_grid_evolve(grid,
            unique_convs.size(),
        ...
        op_params->DglapObj.insert(
            {pineappl_conv_type::PINEAPPL_CONV_TYPE_UNPOL_PDF,
                InitializeDglapObjectsQCDPhys(g,
                    theory.quark_thresholds,
                    true)});
        op_params->DglapObj.insert(
            {pineappl_conv_type::PINEAPPL_CONV_TYPE_POL_PDF,
                InitializeDglapObjectsQCDpolPhys(g,
                    theory.quark_thresholds,
                    true)});
        op_params->DglapObj.insert(
            {pineappl_conv_type::PINEAPPL_CONV_TYPE_UNPOL_FF,
                InitializeDglapObjectsQCDTPhys(g,
                    theory.quark_thresholds,
                    true)});
