...
TheoryCard load_theory_card(const std::string& path) {
    ...
    tc.alpha_qed_ref  = config["AlphaQEDRef"] ? config["AlphaQEDRef"].as<double>() : 0.0;
    tc.lepton_thresholds = config["LeptonThresholds"] ? config["LeptonThresholds"].as<std::vector<double>>() : std::vector<double>{};
