...
GridDef load_grid_def(const std::string &path) {
    ...
    for (const auto &ct : config["ConvolutionTypes"]) {
        ...
        if (s == "UNPOL_PDF")
            def.convolution_types.push_back(PINEAPPL_CONV_TYPE_UNPOL_PDF);
        else if (s == "POL_PDF")
            def.convolution_types.push_back(PINEAPPL_CONV_TYPE_POL_PDF);
        else if (s == "UNPOL_FF")
            def.convolution_types.push_back(PINEAPPL_CONV_TYPE_UNPOL_FF);
        else if (s == "POL_FF")
            def.convolution_types.push_back(PINEAPPL_CONV_TYPE_POL_FF);
...
pineappl_grid *create_grid(const GridDef &def) {
    ...
    for (const auto &ch : def.channels) {
        ...
        pineappl_channels_add(channels,
            combinations,
            flat_pids.data(),
            ch.factors.data());
    }
    ...
    // 8. Create the grid
    pineappl_grid *grid = pineappl_grid_new2(n_bins,
        bin_limits.data(),
        def.orders.size(),
        order_params.data(),
        channels,
...
void set_subgrid(pineappl_grid *grid,
    ...
    pineappl_grid_set_subgrid(grid,
        bin,
        order,
        channel,
        node_values.data(),
